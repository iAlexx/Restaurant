-- Persist order charge snapshots atomically with order creation
...
BEGIN
  ...
  IF p_payload ? 'charges' AND jsonb_array_length(p_payload -> 'charges') > 0 THEN
    FOR v_charge IN
      SELECT value FROM jsonb_array_elements(p_payload -> 'charges')
    LOOP
      INSERT INTO public.order_charges (
        ...
        v_order_id,
        NULLIF(v_charge ->> 'charge_id', '')::uuid,
        v_charge ->> 'name_snapshot',
        v_charge ->> 'calculation_type_snapshot',
        (v_charge ->> 'value_snapshot')::integer,
        (v_charge ->> 'calculated_amount')::integer,
        COALESCE((v_charge ->> 'sort_order_snapshot')::integer, 0)
      );
    END LOOP;
  END IF;
