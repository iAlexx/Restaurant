-- Phase 4: staff manual orders (created_by) + reprint RPC
...
CREATE OR REPLACE FUNCTION public.create_reprint_job(p_order_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  INSERT INTO public.print_jobs (order_id, status, is_reprint)
  VALUES (p_order_id, 'PENDING', true)
...
REVOKE ALL ON FUNCTION public.create_reprint_job(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_reprint_job(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.create_reprint_job(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.create_reprint_job(uuid) TO service_role;
