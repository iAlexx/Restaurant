...
export interface ValidatedManualOrderContext {
  currency_label: string;
  trustedPayload: TrustedOrderPayload;
}

function getManualOrderStatus(
  orderType: ManualOrderInput["order_type"]
): TrustedOrderPayload["status"] {
  return orderType === "DINE_IN" ? "NEW" : "CONFIRMED";
}

export async function validateAndBuildManualPayload(
  supabase: SupabaseClient,
  input: ManualOrderInput,
  createdBy: string
): Promise<ValidatedManualOrderContext> {
